...
public class LatticeHexSectorBackup {
	private int[][] lattice;
	private int[][] age;
			...
			if (t < 8 && params.length > next) {
				...
				new Thread(new Runnable() {
					...
					public void run() {
						...
						LatticeHexSectorBackup lh = new LatticeHexSectorBackup(s);
						lh.setX0(0.000025);// 0.001 = detailed; 0.00032 = balanced; 0.0001 = overall picture
	...
	public LatticeHexSectorBackup(int size, String testName) {
		size += 5;
	...
	public LatticeHexSectorBackup(int size) {
		this(size, null);
	}
		...
		for (int x = 1; x < lattice.length - 1; x++)
			for (int y = 1; y < lattice[x].length - 2; y++) {
				...
				if (lattice[x][y] == 2) {
					...
					growth[x][y] += (ice == 1 || ice == 2 ? alphaFacet : ice == 3 || ice == 4 ? alphaKink : 1) * del
							* delta[x][y];
					if (growth[x][y] >= 1)
						latt[x][y] = 1;
