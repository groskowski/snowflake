...
public class LatticeHexSector2 {
	private int[][] lattice;
	private int[][] age;
	...
	public static void main(String[] args) throws InterruptedException {
		...
		double[] sat = { 0.01, 0.05, 0.1, 0.25, 0.5, 0.75, 1, 1.25, 1.5, 2, 2.5, 3, 4, 5, 7.5, 10 };
		double[] fac = { 0.01, 0.025, 0.05, 0.1, 0.25, 0.5, 0.75, 1, 1.5, 2 };
...
//		for (double sa : sat)
			...
			if (t < 8 && params.length > next) {
				...
				new Thread(new Runnable() {
					...
					public void run() {
						...
						LatticeHexSector2 lh = new LatticeHexSector2(s);
						lh.setX0(0.001);// 0.001 = detailed; 0.00032 = balanced; 0.0001 = overall picture
						lh.setTbMin(0.0001);// Don't change (0.0001 if lost)
						lh.setDeltaFar(param[0]);
						lh.setAlphaFacet(param[1]);
						lh.thread = next;
						System.out.println("Test " + lh.thread);
	...
	public LatticeHexSector2(int size, String testName) {
		size += 5;
	...
	public LatticeHexSector2(int size) {
		this(size, null);
	}
		...
		for (int x = 1; x < lattice.length - 1; x++)
			for (int y = 1; y < lattice[x].length - 2; y++) {
				...
				if (lattice[x][y] == 2) {
					...
					growth[x][y] += (ice == 1 ? 0 : ice == 2 ? alphaFacet : ice == 3 || ice == 4 ? alphaKink : 100000)
							* del * delta[x][y];
					if (growth[x][y] >= 1)
						latt[x][y] = 1;
		...
		for (int x = 1; x < lattice.length - 1; x++)
			for (int y = 1; y < lattice[x].length - 2; y++) {
				...
				if (lattice[x][y] == 0) {
					...
					int ice = count(x, y, 1), vapor = count(x, y, 0), bound = count(x, y, 2);

					double d = ((lattice[x][y - 1] == 0 ? delta[x][y - 1] : 0)
							+ (lattice[x][y + 1] == 0 ? delta[x][y + 1] : 0)
							+ (lattice[x - 1][y] == 0 ? delta[x - 1][y] : 0)
							+ (lattice[x + 1][y] == 0 ? delta[x + 1][y] : 0)
							+ (lattice[x - 1][y - 1] == 0 ? delta[x - 1][y - 1] : 0)
							+ (lattice[x + 1][y + 1] == 0 ? delta[x + 1][y + 1] : 0)) / (vapor);
					if (ice == 1)
	...
	public void save1(String name) {
		...
		for (int x = 1; x < lattice.length - 1; x++) {
			for (int y = 1; y < lattice[x].length - 2; y++) {
				...
				int c = (int) (Math.min(255 * (1 - Math.exp(-delta[x][y])), 255));
				/*
				 * g.setColor(lattice[x][y] == 0 || lattice[x][y] == 2 ? new Color(c, c, c) :
				 * new Color((int) (55 * (Math.exp(-t / 15000f)) + 200 * (Math.exp(-t / 200f))),
				 * (int) (80 * (Math.exp(-t / 15000f)) + 175 * Math.exp(-t / 800f)), (int) (80 +
				 * 175 * (Math.exp(-t / 1500f)))));
				 */
				g.setColor(lattice[x][y] == 0 || lattice[x][y] == 2 ? new Color(c, c, c) : Color.BLUE);
		...
		try {
			...
			ImageIO.write(resize(bi, 4000, 4000), "png", new File(testName + "/gallery/" + name + ".png"));
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
