...
public class Kernel4 {
	int[][] lattice;
	double[][] fb;
	int[][] age;
	double[][] delta;

	public Kernel4(int w, int h) {
		lattice = new int[w][h];
		age = new int[w][h];
	...
	public static void main(String[] args) throws InterruptedException, IOException {
		Kernel4 lat = new Kernel4(size, size);
	...
	private void updateSupersat() {
		...
		double vx = Math.sin(time / 30) / 10;
		double vy = Math.cos(time / 30) / 10;
		vx = 0;
		vy = 0;
		for (int y = 1; y < lattice[0].length - 1; y++)
			for (int x = 1; x < lattice.length - 1; x++) {
	...
	private void save(String name) {
		...
		try {
			ImageIO.write(resize(bi, bi.getWidth() * (600 / size), bi.getHeight() * (600 / size)), "jpg",
					new File(name + ".jpg"));
		} catch (IOException e) {
			e.printStackTrace();
		}
